...
describe('computed (sync)', () => {
  ...
  it('does not re-evaluate when an unrelated key changes', () => {
    ...
    data.b = 200; // unrelated
    expect(fn.mock.calls.length).toBe(callsBefore);
    cleanup();
  });

  it('is idempotent – re-setting the same computed does not duplicate calls', () => {
    const { en, data, cleanup } = setup();
    const fn = vi.fn(() => data.x * 2);

    data.x = 3;
    data.doubled = en.computed(fn);
    data.doubled = en.computed(fn); // re-set
...
describe('computed (async)', () => {
  ...
  it('discards stale async results on rapid changes', async () => {
    ...
    // Use manual resolve handles so we control the order of resolution
    // without needing any timer APIs.
    ...
    // Trigger the second (faster) call
    data.input = 'b';

    // Resolve second first, then first – second result should win
